...
############ settings ##############
in_file <- "software-institute.csv" #name of file to import
...
cat("* Converting date\n")
#from 10 August 2010 4:15 p.m. -> 2010-08-10 16:15:00
pubdate <- str_replace(raw_data$published, "am", " AM")
pubdate <- str_replace(pubdate, "pm", " PM")
...
fauthor <- fullauthor(content)
#remove author from content
content <- body_no_author(content)

#add to new data frame
clean_data <- data.frame(url,title,content,fauthor,pubdate)
