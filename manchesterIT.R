...
############ settings ##############
in_file <- "manchesterIT.csv" #name of file to import
...
cat("* Converting date\n")
#convert from "1 Jul 2019, 9 am" -> 2019-07-01 09:00:00
pubdate <- convdate(raw_data$published)
...
#clean text
cat("* Starting to process title, subtitle and body\n")
title <- cleantext(raw_data$title)
subtitle <- cleantext(raw_data$subtitle)
body <- cleantext(raw_data$content)
  
#join subtitle to content
content <- trimws(paste(subtitle, body))
